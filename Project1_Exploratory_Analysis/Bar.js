...
d3.csv("./Data/Data_Final.csv", d3.autoType).then(data1 => 
    {
...
.domain(Array.from(new Set(state1.data1.map(d=> d.Region))))
...
.domain([0, d3.max(state1.data1, d=>d.Fertility_rate19)]) //take 0-1435(max value of count)
...
const colorScale1 = d3.scaleOrdinal()
  ...
  .domain(Array.from(new Set(d3.map(state1.data1, d=>d.Enitity))))
  .range(d3.schemeSet2);
...
.attr("height", d=>innerHeight-y(d.Fertility_rate19))
.attr("x", d=>x(d.Region))
.attr("y", d=>y(d.Fertility_rate19))
.style("fill", d=>colorScale1(d.Entity))
.attr("class", "bubbles")
.attr("text-anchor", 'middle')
}
// .attr("width", d => d.x)
